...
puts school_hash.group_by { |data| data[:student] }.map { |key, value|
       Hash[ key,
             value.map { |key, _value| Hash[key[:subject], [key[:mark1], key[:mark2]]] }
       ]
     }
...
puts school_hash.select do |element|
  (element[:mark1].to_i + element[:mark2].to_i) / 2 == average_maximum
end.group_by { |element| element[:teacher] }.map do |key, value|
  { teacher: key, student: value.map do |element|
                             element[:student]
                           end, rate: average_maximum }
end
